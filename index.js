...
import { ActionRowBuilder, ButtonBuilder, ButtonStyle, Client, GatewayIntentBits, ModalBuilder, Partials, TextInputBuilder, TextInputStyle } from 'discord.js';
...
async function getActionName(actionId, onlyEnabled = true) {
    let sanitizedUrl = sanitizeUrl(process.env['STREAMER_BOT_HTTP_SERVER']) + 'GetActions';
    ...
    if (response.status == 200) {
        ...
        return new Promise.reject('Non 200 response from Streamer.Bot\'s HTTP Server!')
    }
}

async function doAction(actionId, actionName, args = null) {
    let sanitizedUrl = sanitizeUrl(process.env['STREAMER_BOT_HTTP_SERVER']) + 'DoAction';
    let body = null;
    if (args) {
        body = JSON.stringify({
            action: {
    ...
    if (response.status == 204) {
        ...
        return new Promise.reject('Non 204 response from Streamer.Bot\'s HTTP Server!')
    }
}
...
client.on("interactionCreate", async (interaction) => {
    ...
    if (interaction.isButton()) {
        if (interaction.customId.startsWith('delete')) {
            ...
            let actionName = await getActionName(actionId).catch(err => console.warn);
            ...
            let modal = new ModalBuilder().setTitle('Timeout User').setCustomId('timeoutModal').setComponents(
                new ActionRowBuilder().setComponents(new TextInputBuilder().setCustomId('timeoutDuration').setLabel('Timeout Duration in Seconds').setMaxLength(10).setMinLength(1).setPlaceholder('Timeout Duration in Seconds').setStyle(TextInputStyle.Short)),
                new ActionRowBuilder().setComponents(new TextInputBuilder().setCustomId('timeoutReason').setLabel('Timeout Reason').setRequired(false).setPlaceholder('Timeout Reason').setStyle(TextInputStyle.Paragraph))
            );
            ...
            let modal = new ModalBuilder().setTitle('Ban User').setCustomId('banModal').setComponents(
                new ActionRowBuilder().setComponents(new TextInputBuilder().setCustomId('banReason').setLabel('Ban Reason').setRequired(false).setPlaceholder('Ban Reason').setStyle(TextInputStyle.Paragraph))
            );
            await interaction.showModal(modal);
            let submitted = await interaction.awaitModalSubmit({
    ...
    ws.onmessage = async event => {
        ...
        if (wsdata.event.source == 'Twitch') {
            if (wsdata.event.type == 'ChatMessage') {
                ...
                let nameToPost = displayName.toLowerCase() == username ? displayName : `${displayName} (${username})`;
                ...
                if (dcChannel) {
                    if (dcChannel.isTextBased()) {
                        ...
                        let deleteBtn = new ButtonBuilder().setCustomId(`delete${msgId}`).setLabel('Delete').setStyle(ButtonStyle.Success);
                        let timeoutBtn = new ButtonBuilder().setCustomId(`timeout${username}`).setLabel('Timeout').setStyle(ButtonStyle.Danger);
                        let banBtn = new ButtonBuilder().setCustomId(`ban${username}`).setLabel('Ban').setStyle(ButtonStyle.Danger);
                        let actionRow = new ActionRowBuilder().addComponents(deleteBtn, timeoutBtn, banBtn);
                        dcChannel.send({
